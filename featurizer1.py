    ...
    def get_pos_features(self, cleaned_text):
        ...
        with nlp.disable_pipes(*['lemmatizer', 'ner']):
            ...
            for doc in nlp.pipe(cleaned_text, batch_size=1000, n_process=n_process):\
                ...
                ex=[token.text for token  in doc if token.text=='!']
                matches = matcher(doc)
                mo=[doc[start:end].text for match_id, start, end in matches]
                ...
                exclamation_count.append(len(ex))
                money_count.append(len(mo))
                
        
        # Stack the count lists vertically to form a 2D numpy array
        return np.transpose(np.vstack((noun_count, aux_count, verb_count, adj_count,exclamation_count,money_count)))
    ...
    def transform(self, X, y=None):
        ...
        try:
            ...
            if self.pos_features:
                ...
                feature_names.extend(['noun_count', 'aux_count', 'verb_count', 'adj_count','exclamation_count','money_count'])
            else:
